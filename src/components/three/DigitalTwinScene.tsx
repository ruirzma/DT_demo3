...
import { Environment, Html, Line, OrbitControls, Tube, useGLTF } from '@react-three/drei';
...
function Asset({url,pos,scale=0.4,rot=0}:{url:string;pos:[number,number,number];scale?:number;rot?:number}){const gltf=useGLTF(url);return <primitive object={gltf.scene} position={pos} scale={scale} rotation={[0,rot,0]} />}
...
export default function DigitalTwinScene(props:Props){
  ...
  return <Canvas camera={{position:cameraPos as [number,number,number],fov:42}} shadows gl={{antialias:true}}>
    <color attach='background' args={['#06101b']}/><fog attach='fog' args={['#07111a',8,20]}/>
    ...
    <Suspense fallback={null}>
      <Asset url='https://rawcdn.githack.com/KhronosGroup/glTF-Sample-Models/master/2.0/Truck/glTF-Binary/Truck.glb' pos={[-3.6,0.12,2.9]} scale={0.25} rot={0.8}/>
      <Asset url='https://rawcdn.githack.com/KhronosGroup/glTF-Sample-Models/master/2.0/BarramundiFish/glTF-Binary/BarramundiFish.glb' pos={[-2.3,0.12,2.2]} scale={0.01} rot={0}/>
      <Asset url='https://rawcdn.githack.com/KhronosGroup/glTF-Sample-Models/master/2.0/Lantern/glTF-Binary/Lantern.glb' pos={[2.8,0.7,2.9]} scale={0.3} rot={0}/>
    </Suspense>
